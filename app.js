(() => {
  ...
  function renderCronograma(items) {
    ...
    $('cronogramaTable').innerHTML = rows.length ? `<table><thead><tr><th>Fecha</th><th>Hora</th><th>Llamado</th><th>Espacio</th><th>Plan</th><th>Año</th><th>Aula</th><th>Docentes</th><th>Origen</th></tr></thead><tbody>${rows.map(r => `<tr><td>${esc(r.fecha)}</td><td>${esc(r.horaInicio)}</td><td>${esc(r.llamado)}</td><td><strong>${esc(r.espacio)}</strong><br><small>${esc(r.codigoPe)}</small></td><td>${esc(r.plan)}</td><td>${esc(r.anio)}</td><td>${esc(r.aula)}</td><td>${esc(r.docentes)}</td><td><span class="badge ${r.ajustado === 'SI' ? 'warn' : ''}">${r.ajustado === 'SI' ? 'MODIFICADA' : 'BASE'}</span></td></tr>`).join('')}</tbody></table>` : '<div class="empty">No hay mesas para mostrar.</div>';
